...
@section('head')
    ...
    <style>
        ...
        .drag-handle svg {
            pointer-events: none;
            /* Make sure clicks go through to the handle */
        }
        ...
        <div class="flex flex-wrap gap-6">
            ...
            <div class="flex-1">
                ...
                @php
                    ...
                    // Ensure both category variables are always strings to prevent htmlspecialchars errors
                    $selectedMainCategory = (string) $selectedMainCategory;
                    $selectedStaffCategory = (string) $selectedStaffCategory;
                @endphp
                <select name="main_category" id="main_category" class="input-text">
                    @foreach ($mainCategories as $category)
                        <option value="{{ $category['slug'] }}"
                            {{ $selectedMainCategory == $category['slug'] ? 'selected' : '' }}>
                            {{ $category['title'] }}
                        </option>
                    @endforeach
                </select>
            ...
            <div class="flex-1">
                ...
                <select name="staff_category" id="staff_category" class="input-text" onchange="this.form.submit()"
                    {{ empty($selectedMainCategory) ? 'disabled' : '' }}>
                    <option value="">All Staff</option>
                    @foreach ($staffCategories as $category)
                        <option value="{{ $category['slug'] }}"
                            {{ $selectedStaffCategory == $category['slug'] ? 'selected' : '' }}>
                            {{ $category['title'] }}
                        </option>
                    @endforeach
                </select>
                        ...
                        <td class="flex items-center">
                            ...
                            @php
                                ...
                                if (is_array($picture) && !empty($picture)) {
                                    $firstPicture = reset($picture);
                                    if (is_object($firstPicture) && method_exists($firstPicture, 'url')) {
                                        $pictureUrl = $firstPicture->url();
                                    }
                                } elseif (is_object($picture) && method_exists($picture, 'url')) {
                                    $pictureUrl = $picture->url();
                                }
                            @endphp
                            @if ($pictureUrl)
                                <img src="{{ $pictureUrl }}" alt="{{ $item->title }}"
                                    class="w-20 h-20 rounded-full mr-3 object-cover" style="width: 60px; height: 60px;"
                                    loading="lazy">
                            @else
                                <div class="w-20 h-20 rounded-full bg-gray-200 flex items-center justify-center text-gray-500 mr-3"
                            ...
                            @endif
                            <span>{{ is_array($item->title) ? '' : ($item->title ?? '') }}</span>
                        </td>
                        <td>{{ is_array($item->get('position')) ? '' : ($item->get('position') ?? '') }}</td>

                        <td>
                            <span class="px-2 py-1 rounded-full text-xs font-medium ">
                                {{ is_array($item->get('staff_category')) ? '' : ($item->get('staff_category') ?? '') }}
                            </span>
                        </td>
                ...
                @empty
                    <tr>
                        <td colspan="4" class="text-center py-4">
                            ...
                            <a href="{{ cp_route('complex-collection-ordering.create') }}" class="text-blue-500">Add
                                one</a>.
                        </td>
                    </tr>
                @endforelse
            ...
            <div class="flex items-center space-x-2">
                ...
                @else
                    <a href="{{ $items->previousPageUrl() }}"
                        class="px-3 py-1 rounded bg-white border border-gray-300 hover:bg-gray-50">
                        &laquo; Previous
                    </a>
                @endif
